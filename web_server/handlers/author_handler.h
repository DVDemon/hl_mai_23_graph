...
public:
                       ...
                       HTTPServerResponse &response)
    {
        ...
        if (form.has("login"))
        {
            ...
            {
                std::string login_add = form.get("add_friend").c_str();
                try
                {
                    database::Author result = database::Author::read_by_login(login);
                    result.add_friend(login_add);
                    ostr << "{ \"result\": \"true\" }";
                    return;
                }
                catch (...)
            ...
            else if (form.has("remove_friend"))
            {
                std::string login_remove = form.get("remove_friend").c_str();
                try
                {
                    database::Author result = database::Author::read_by_login(login);
                    result.remove_friend(login_remove);
                    ostr << "{ \"result\": \"true\" }";
                    return;
                }
                catch (...)
